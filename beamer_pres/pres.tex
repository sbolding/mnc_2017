...
% \item[] This work increases time-integration accuracy \\ \colG{of the radiation intensity in optically thin regions}
%    \end{itemize}}
%\end{frame}
...
\begin{figure}[h]
    ...
    \centering
        \begin{tikzpicture}[scale=0.702, every node/.style={transform shape}]
            ...
            \draw (1.0,0.4) -- (1.0,0.6) node[below, pos=0.4] {$t^{n-1/2}$};
            \draw (5.90,0.4) -- (5.90,0.6) node[below, pos=0.4] {$t^{n+1/2}$};
    ...
    \vspace{0.1in}
        \begin{tikzpicture}[scale=0.702, every node/.style={transform shape}]
            ...
            \node[anchor=west] at (5.9,0.99) {${\tilde{I}_{HO}^{n+1/2}(x,\mu)}$};
        \end{tikzpicture}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Comparisons have the same total number of histories
        \\\colG{IMC results from Jayenne (LANL code)}
    \item One HO solve per time step, with two LO solves
    \item Figure of Merit as estimate of efficiency:\\ \vspace{0.1in} \boxed{\text{FOM}=\frac{\displaystyle 1}{\displaystyle
        \left(\frac{\ds\|\sigma(\phi_i)\|_2}{\ds\|\phi_i\|_2}\right)^2 N_{\text{total}}}}\\\vspace{0.1in}{ Normalized so FOM for IMC is unity}
    \end{itemize}
    }
\end{frame}
