...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{frame}
 \frametitle{ECMC uses a \colb{projection} $\tilde I(x,\mu,t)$ onto a \\
 space-angle-time FE mesh to represent the solution}
     \begin{itemize}
         \item linear-discontinuous FE projection in $x$ and $\mu$
         \item Tried three different $t$ spaces: SDD, LDD, LD
            \end{itemize}
\begin{figure}[h]
    \centering
...
unknowns with HO information}
    \begin{enumerate}
        \item Assume LD spatial closure  for $T(x)$, $T^4(x)$, and
            $\phi(x)$
    \item Eliminate space-angle moments of $I^{n+1/2}_{LO}$  in terms of
        \textbf{time-averaged} moments $\overline I_{LO}$, e.g.,
            \begin{equation*}
                \mom{\phi}_{L,i}^{n+1/2,+} = \highlight{\gamma^{HO,+}_{L,i}} {\mom{\overline{\phi}}}^+_{L,i} 
            \end{equation*}
        \item Coupled equations are solved with Newton's method \\ \colG{for time-averaged moment unknowns}
        \item Use time closure to advance to the next time step
       \end{enumerate}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item HOLO method is written in stand-alone \textrm{C}\texttt{++}  \\
