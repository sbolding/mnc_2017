...
estimate of the error in $\tilde{I}^{(m)}$, i.e., $\tilde{\epsilon}^{(m)} = \B
...
The LDFE projection $\overline{\epsilon}$ and $\epsilon^{n+1}$ are computed using
generalizations of volumetric path-length and particle density
estimators~\cite{wollaber_review}, respectively.  The estimators are weighted by appropriate basis
...
problems.  This problem has the same material properties as the previous, except
